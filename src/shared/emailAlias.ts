...
async function createSimpleLoginAlias(
	...
	try {
		const response = await fetch(`${SIMPLELOGIN_API_BASE}/alias/random/new`, {
			...
			body: JSON.stringify({
				note: "Generated by Cloak & Dagger",
			}),
		});

		if (!response.ok) {
			console.warn(
				`[Cloak] SimpleLogin API error: ${response.status} ${response.statusText}`,
			);
			return null;
		}
		...
		console.warn("[Cloak] SimpleLogin request failed:", err);
		return null;
	}
}
	...
	if (alias.source === "simplelogin") {
		...
		if (tokens.simplelogin) {
			try {
				...
				console.warn("[Cloak] Failed to delete alias from SimpleLogin API");
			}
		}
	}
